...
class alt_dynamic_environment(py_environment.PyEnvironment):

    def __init__(self, actions, gamma, threshold, links, min=0.1, max=0.1):
        self.links = links
        self.gamma = gamma
        self.threshold = threshold
        self.max = max
        self.min = min
        self.delta = (max - min) / links
    ...
    def _step(self, action):
        ...
        self.gamma = self.min + len([i for i in self.state if i != 0]) * self.delta
...
# environment = alt_dynamic_environment(actions, gamma, threshold, links)
