...
flags.DEFINE_string(
    'root_dir',
    os.path.join(os.path.curdir, "ppo"),
...
flags.DEFINE_integer(
    'num_eval_episodes', 32, 'The number of episodes to run eval on.'
)
flags.DEFINE_boolean(
    'use_rnns', False, 'If true, use RNN for policy and value function.'
)
# use_rnns = True                 # NOTE: I HAVE NO CLUE WHAT THIS DOES!
FLAGS = flags.FLAGS
    ...
    with tf.compat.v2.summary.record_if(
        ...
        tf_env = tf_py_environment.TFPyEnvironment(
            parallel_py_environment.ParallelPyEnvironment(
                [lambda: TimeLimit(discrete_environment(actions, decay_rate, threshold, links), duration=timeout)] * num_parallel_environments
            )
        )
...
def main(_):
  ...
  train_eval(
      ...
      actor_fc_layers=(256, 64, 32),   # NOTE: I HAVE NO CLUE WHAT THIS DOES!
      value_fc_layers=(256, 64, 32),   # NOTE: I HAVE NO CLUE WHAT THIS DOES!
